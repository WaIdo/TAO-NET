...
parser.add_argument('--sampled_num', type=int, default=4000)
...
if __name__ == "__main__":
    # Step 0: configure logging and fix random seeds
    ...
    # Step 1: select the runtime device
    ...
    if torch.cuda.is_available() and config.gpu_mode > 0:
        ...
        if config.gpu_mode == 3:
            ...
            # Single-GPU mode (prefer setting CUDA_VISIBLE_DEVICES outside)
            device = torch.device('cuda')
            print('Using one GPU: %s.' % device)
            logger.info('Using one GPU: %s.' % device)
        else:
            assert config.gpu_mode == 2
            # DataParallel mode with automatic GPU selection by free memory
    ...
    print(config)
    # Step 3: ensure the checkpoint directory exists
